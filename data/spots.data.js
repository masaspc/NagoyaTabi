  ...
  NT.spots = [
    ...
    { id:'pokecen', name:'ポケモンセンターナゴヤ', area:'栄・大須', category:'ショッピング',
      ...
      trivia:[
        ...
        '混雑期は入場に当日整理券が要る。対象日は公式サイトのお知らせで事前に告知される'
      ],
      tips:[
        '西館ではなく東館2Fなので入口を間違えないこと',
        /* 8/12に実際にかかった。営業時間だけ見て「開店に行けば入れる」と組むと外す */
        '混雑期は当日整理券制。8月は土日祝に加えてお盆の混雑予想日（8/10-16など）が対象で、' +
          '水曜の8/12も含まれていた。配布は9:00から（開店の1時間前）で、10:00に着いてもその場では入れない',
        '整理券をもらったら、書かれた回の時刻まで館内で待つ。屋外で待たない',
        '滞在時間には会計の列を足す。整理券が出る日はレジも混む'
      ],
      route:[
    ...
    /* 朝のきしめん2軒（8/12の朝食）。住よしと違ってどちらも改札の外なので、
       入場券も乗車券も要らずに座って食べられる。7:00開店・年中無休。
       運営会社（ジェイアール東海フードサービス）の公式ページは確認時点で
       こちらの回線から開けなかったため、営業時間は複数の情報源が一致した
       7:00-22:00 を採り、朝の定食の内容・提供時間だけは店に電話して確かめる
       前提で tips に残してある。 */
    { id:'ekikama-taiko', name:'驛釜きしめん 太閤通り', area:'名古屋駅', category:'食事',
      indoor:true, shade:true,
      station:'JR名古屋駅 太閤通口', walk:'名古屋うまいもん通り（太閤通口）内。改札外',
      hours:'7:00-22:00（L.O.21:30）',
      closed:'年中無休',
      fee:'きしめん 600-900円程度（朝の定食 800円前後）', stay:35,
      lat:35.1703, lng:136.8809, tel:'052-526-0147',
      map:'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent('驛釜きしめん 太閤通り 名古屋うまいもん通り'),
      official:'https://www.jrt-food-service.co.jp/store/348/',
      trivia:[
        '7:00開店。名古屋駅で朝からきしめんを座って食べられる数少ない店のひとつ',
        '改札の外（名古屋うまいもん通り 太閤通口）にあるので、入場券も乗車券も要らない',
        '朝の時間帯には、ミニサイズのきしめんに天むすが付く朝の定食がある',
        '小麦粉・塩・水だけで打つ麺を掲げる。立ち食いではなく席に座って食べられる'
      ],
      tips:[
        '朝の定食（朝きしセット）は提供時間が朝の数時間に限られる。内容と時間は当日 052-526-0147 で確かめるのが確実',
        'エスカ・新幹線改札と同じ太閤通口側なので、荷物をコインロッカーに入れる動線とまとめられる',
        '地下鉄へ向かうなら、食べたあと東の桜通口側へ抜ける（駅の端から端で徒歩7-8分みておく）'
      ],
      route:[
        '太閤通口の名古屋うまいもん通りへ入る（新幹線改札の外側）',
        '店頭で朝の定食かきしめんを選んで注文する',
        '席に座って食べる。混んでいなければ20-30分で出られる',
        '食べ終えたら、地下鉄へ向かう場合は桜通口側へ移動する'
      ],
    ...
    { id:'ekikama-chuo', name:'驛釜きしめん 中央通り', area:'名古屋駅', category:'食事',
      indoor:true, shade:true,
      station:'JR名古屋駅 中央コンコース', walk:'中央改札口から徒歩1分。改札外',
      hours:'7:00-22:00（L.O.21:30）',
      closed:'年中無休',
      fee:'きしめん 600-900円程度', stay:35,
      lat:35.1709, lng:136.8823, tel:'052-569-0282',
      map:'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent('驛釜きしめん 中央通り 名古屋駅'),
      official:'https://www.jrt-food-service.co.jp/store/338/',
      trivia:[
        '同じ驛釜きしめんの中央コンコース側の店。こちらも7:00開店',
        '2025年5月に同じ通り沿いの隣接区画へ移転して開き直した',
        '中央改札口から徒歩1分で、桜通口・地下鉄側に近い'
      ],
      tips:[
        '太閤通り店が混んでいたらこちら。駅の中央付近なので地下鉄へ出るのが速い',
        '朝の定食の有無は店舗で異なることがある。052-569-0282 で確かめられる'
      ],
      route:[
        '中央コンコースの名古屋うまいもん通りへ入る',
        '店頭できしめんを注文し、席で食べる',
        '食べ終えたら桜通口側の地下鉄乗り場へ向かう'
      ],
