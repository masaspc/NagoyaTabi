...
(function (w) {
  ...
  /* 模式図の座標。地理ではなく路線図としての分かりやすさを優先する。
     ...
     これに伴う座標の調整は伏見だけ:
       ...
       - 大須観音・上前津・金山・熱田神宮伝馬町・矢場町・市役所・栄は
         レビュー1で調整済みの座標のまま
     ...
     【駅名ラベルとピンの重なりの修正】駅名は text-anchor:end で
     x=station.x-11 を右端として左へ伸びる。ピンの帯は駅の右
     (x+13〜x+117)に伸びる。この2つは通常は互いに逆方向なので重ならないが、
     「駅Bのラベルが駅Aのピン帯のx範囲に届くほど近く、かつ駅Bの駅名の
     y(=B.y+4)が駅Aのピン帯のy範囲とも重なる」場合にだけ衝突する
     （ピンは駅名より後に描画されるため、ラベルの左側1〜2文字がピンの
     カードで隠れる）。該当したのは2駅:
       - 丸の内(200,90→190,45): ラベル帯(およそx:146-179)が名古屋の
         ピン帯(x:73-177)とx方向に重なる。x はほぼそのまま（190、東山線
         との交差を避ける位置は既に確保済み）、yだけ名古屋のピン帯
         (y:68.5-158.5)の外、上へ出した
       - 大曽根(390,50→390,90): ラベル帯(およそx:346-379)は市役所・栄
         どちらのピン帯(どちらもx:263-367。同じ列なので x 帯は共通)とも
         x方向に重なってしまう。なので y を、市役所のピン帯(y:28.5-72.5)
         にも栄のピン帯(y:128.5-218.5)にもかからない、その間の隙間
         (およそy:72.5-128.5)の中央付近(y=90)に置いた
     どちらも「駅どうしの縦方向の間隔を広げる」方針で解決している
     （text-anchor を左側で反転する案は、丸の内・大曽根はどちらも自分の
     ピンを持つため、ラベルを右に反転すると今度は自分のピン帯とぶつかる
     ので採らなかった） */
  var ST = {
    nagoya:     { x:  60, y:  90, label: '名古屋' },
    marunouchi: { x: 190, y:  45, label: '丸の内' },
